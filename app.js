...
io.on('connection', function(socket) {
  ...
  socket.on('message', function(msg) {
    ...
    if (msg.target == "All") {
      ...
      io.sockets.sockets.get(clients[msg.target]).emit('message', {
        "source": srcUser,
        "message": msg.message,
        "target": msg.target
      });
...
function userJoined(uName) {
  Object.keys(socketsOfClients).forEach(function(sId) {
    io.sockets.sockets.get(sId).emit('userJoined', { "userName": uName });
  });
}
...
function userNameAvailable(sId, uName) {
  setTimeout(function() {
    ...
    io.sockets.sockets.get(sId).emit('welcome', { "userName": uName, "currentUsers": JSON.stringify(Object.keys(clients)) });
  }, 500);
}

function userNameAlreadyInUse(sId, uName) {
  setTimeout(function() {
    io.sockets.sockets.get(sId).emit('error', { "userNameInUse": true });
  }, 500);
}
